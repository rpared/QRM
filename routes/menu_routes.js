...
router.post("/add_menu_item", upload.single('item_photo'), async (req, res) => {
    ...
    try {
        const restaurant = await Restaurant.findById(resta_profile_id);
        if (!restaurant) {
            console.error(`Restaurant with ID ${resta_profile_id} not found`);
            return res.status(404).send("Restaurant not found");
        }

        if (req.file) {
            item_photo = req.file.location; // S3 file location
        }
        ...
        const newItem = new MenuItem({
            ...
            item_category,
            item_name,
            item_photo, // Ensure this contains the S3 URL
            item_description,
...
router.get("/restaurant/:restaurantId/menu/:menuItemId/edit", async (req, res) => {
  ...
  router.get("/restaurant/:restaurantId/styles", async (req, res) => {
      ...
      try {
      const restaurant = await Restaurant.findById(restaurantId); // Needs the Restaurant model
      if (!restaurant) {
          return res.status(404).send("Restaurant not found");
      }
